...
class _AdminScreenState extends State<AdminScreen> {
  ...
  String _collectionName(PlaceCategory c) {
    switch (c) {
      case PlaceCategory.site: return 'sites';
      case PlaceCategory.hotel: return 'hotels';
      case PlaceCategory.resto: return 'restaurants';
      case PlaceCategory.event: return 'events';
      case PlaceCategory.entreprise: return 'business';
      case PlaceCategory.shopping: return 'shopping';
    }
  }
  ...
  Widget _headerTitle(ThemeData theme) {
    return Column(
      ...
      children: [
        Text('Tableau de bord', style: theme.textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.w900, fontSize: 26)),
        const SizedBox(height: 4),
        Text('Gerez le contenu de Kin City Guide', style: theme.textTheme.bodyMedium?.copyWith(color: theme.textTheme.bodySmall?.color?.withOpacity(0.75), fontWeight: FontWeight.w500)),
      ],
    );
  }
  ...
  Widget _statsCard(ThemeData theme, {required int published, required int drafts, required int reels}) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: const LinearGradient(colors: [Color(0xFF0B7A4A), Color(0xFF14B36B)], begin: Alignment.topLeft, end: Alignment.bottomRight),
        borderRadius: BorderRadius.circular(18),
        boxShadow: [BoxShadow(color: _green.withOpacity(0.28), blurRadius: 18, offset: const Offset(0, 10))],
      ),
      child: Row(
        children: [
          Expanded(child: _statItem(theme, value: published.toString(), label: 'Publies', textColor: textColor, icon: Icons.public)),
          _divider(),
          Expanded(child: _statItem(theme, value: drafts.toString(), label: 'Brouillons', textColor: textColor, icon: Icons.feed_outlined)),
          _divider(),
          Expanded(child: _statItem(theme, value: reels.toString(), label: 'Reels', textColor: textColor, icon: Icons.play_circle_outline)),
        ],
      ),
    );
  }

  Widget _divider() => Container(width: 1, height: 44, margin: const EdgeInsets.symmetric(horizontal: 8), color: Colors.white.withOpacity(0.28));
      ...
      children: [
        ...
        Expanded(
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text(value, style: theme.textTheme.titleLarge?.copyWith(color: textColor, fontWeight: FontWeight.w900, height: 1.0)),
              const SizedBox(height: 2),
              Text(label, style: theme.textTheme.bodySmall?.copyWith(color: textColor.withOpacity(0.90), fontWeight: FontWeight.w600, fontSize: 11)),
            ],
          ),
        ),
      ],
  ...
  Widget _sectionLabel(ThemeData theme, String text) {
    return Text(text, style: theme.textTheme.labelSmall?.copyWith(letterSpacing: 1.2, fontWeight: FontWeight.w800, color: theme.textTheme.bodySmall?.color?.withOpacity(0.65)));
  }
  ...
  Widget _menuButton(ThemeData theme, {required IconData icon, required String label, required VoidCallback onTap, int? badge}) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [if (theme.brightness == Brightness.light) BoxShadow(color: Colors.black.withOpacity(0.04), blurRadius: 10, offset: const Offset(0, 6))],
      ),
      child: ListTile(
        onTap: onTap,
        leading: CircleAvatar(backgroundColor: _green.withOpacity(0.10), child: Icon(icon, color: _green)),
        title: Text(label, style: theme.textTheme.bodyMedium?.copyWith(fontWeight: FontWeight.w700)),
        trailing: Row(
  ...
  Widget _profileMenu(ThemeData theme) {
    ...
    return PopupMenuButton<String>(
      tooltip: 'Compte',
      onSelected: (v) { if (v == 'logout') _signOut(); },
      itemBuilder: (context) => [
        const PopupMenuItem<String>(value: 'logout', child: Row(children: [Icon(Icons.logout, size: 18), SizedBox(width: 10), Text('Deconnexion')])),
      ],
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.end,
        children: [
          Container(
            decoration: BoxDecoration(shape: BoxShape.circle, border: Border.all(color: Colors.white, width: 2), boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.10), blurRadius: 10, offset: const Offset(0, 6))]),
            child: CircleAvatar(
              radius: 22,
              backgroundColor: _green,
              backgroundImage: (user?.photoURL != null && user!.photoURL!.isNotEmpty) ? NetworkImage(user.photoURL!) : null,
              child: (user?.photoURL == null || user!.photoURL!.isEmpty) ? const Icon(Icons.person, color: Colors.white) : null,
            ),
          ),
          const SizedBox(height: 6),
          if ((displayName != null && displayName.isNotEmpty) || (email != null && email.isNotEmpty))
            Container(
              padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 6),
              decoration: BoxDecoration(
                color: theme.brightness == Brightness.light ? Colors.white.withOpacity(0.92) : theme.cardColor.withOpacity(0.92),
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.end,
                children: [
                  if (displayName != null && displayName.isNotEmpty) Text(displayName, style: theme.textTheme.labelMedium?.copyWith(fontWeight: FontWeight.w800)),
                  if (email != null && email.isNotEmpty) Text(email, style: theme.textTheme.labelSmall?.copyWith(color: theme.textTheme.bodySmall?.color?.withOpacity(0.70), fontWeight: FontWeight.w600)),
                ],
              ),
            ),
        ],
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(
        ...
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text('Choisir une categorie', style: theme.textTheme.titleLarge?.copyWith(fontWeight: FontWeight.bold)),
          const SizedBox(height: 20),
          ...PlaceCategory.values.map((category) => ListTile(
